...
export class ManageUserComponent {
  ...
  actionFn(value: any) {
    ...
    if (value === '2') {
      ...
      this.UserService.deleteUser(this.listUsers).subscribe((data: any) => {
        this.getUser();
  ...
  updateArticles(item: any) {
    ...
    let listUsers: any[] = [];
    listUsers.push(item.id);
    this.NewService.updateArticle(null, listUsers).subscribe((data: any) => {
      this.getUser();
  ...
  handlePageEvent(e: any) {
    ...
    this.queries.page = this.pageIndex + 1;
    this.NewService.getAllByAd({ ...this.queries }).subscribe((data: any) => {
      this.users = data.rows;
      this.length = data.count;
    });
  }
