...
export class ManageArticlesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.UserService.getAll().subscribe((data: any) => (this.user = data.rows));
  ...
  checkShowColoumns(type: string) {
    return this.configUser.coloumns.some((coloumn: any) => coloumn === type);
  }
  getArticles() {
    console.log(this.formFilter.value.created_user_id);
  ...
  updateArticles(item: any) {
    ...
    this.NewService.updateArticle(null, listArticles).subscribe((data: any) => {
      this.getArticles();
  ...
  handlePageEvent(e: any) {
    ...
    if (this.order.length > 0) this.queries.order = JSON.stringify(this.order);
    this.queries.page = this.pageIndex + 1;
    this.getArticles();
  }
